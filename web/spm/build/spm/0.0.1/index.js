...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag("msg-stu",'<div if="{!hidden}"> <input type="text" name="content"> <input onclick="{sendToStu}" type="button" value="发送"> <div class="msg_bubble_container" each="{msgs}"> <span class="{msg_bubble:true, msg_bubble_l:from.user === __page.user._id, msg_bubble_r:from.user != __page.user._id}"><span>{from.name}</span>：<span>{content}</span></span> </div> </div>',function(){var n=nest.presentable(this),e=domain.action("MsgSingleSendAction"),t=domain.action("MsgStuInitDataAction");n.sendTo={},this.on("open",function(e){n.clazzId=e.clazzId,_onOpen(),n.trigger("view-route-to")}),this.on("mount",function(){}),_onOpen=function(){t.newInvocation({clazzId:n.clazzId}).onDone(function(e){n.msgs=e.msgs,n.sendTo.userId=e.teacher.user,n.update()}).execute()}.bind(n),this.sendToStu=function(t){e.newInvocation({to:n.sendTo.userId,content:n.content.value}).onDone(function(e){console.log(e),n.msgs.unshift(e),n.update()}).execute(),t.stopPropagation()}.bind(this)});
