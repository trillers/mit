...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag("clazz-notifies",'<div if="{util.teacherOrNot() && !hidden}"> <input name="content" type="text"><input onclick="{sendToClazz}" type="button" value="发送"> </div> <div class="chat_card" if="{!hidden}"> <clazz-notify class="chat_notify" each="{msgs}" msgid="{_id}"></clazz-notify> </div>','clazz-notifies .chat_card, [riot-tag="clazz-notifies"] .chat_card{ background: #ECECEC; } clazz-notifies .chat_card .chat_notify, [riot-tag="clazz-notifies"] .chat_card .chat_notify{ display: block; background: white; margin-bottom: 10px; border-bottom: 1px solid #E9E9E9; min-height: 60px; line-height: 60px; }',function(){function t(t){n.msgs=t,n.trigger("view-route-to")}var n=nest.presentable(this),i=domain.action("MsgInClazzLoadAction"),o=domain.action("MsgInClazzAddAction");n.msgs=[],n.clazzId="";var a=function(t){n.clazzId=t.clazzId,i.execute({channel:t.clazzId})}.bind(n),c=function(){}.bind(n);this.on("open",function(t){a(t)}),this.on("mount",function(){i.onDone(t)}),this.sendToClazz=function(t){var i=o.newInvocation({channel:n.clazzId,content:n.content.value});i.onDone(function(t){n.msgs.unshift(t),n.update()}).execute(),t.stopPropagation()}.bind(this),this.on("refresh",function(){c()})});
	...
	riot.tag("msg-usr",'<div if="{!hidden}"> <input type="text" name="content"> <input onclick="{sendToStu}" type="button" value="发送"> <div class="msg_bubble_container" each="{msgs}"> <span class="{msg_bubble:true, msg_bubble_l:from.user === __page.user._id, msg_bubble_r:from.user != __page.user._id}"><span>{from.name}</span>：<span>{content}</span></span> </div> </div>','msg-usr .msg_bubble_container, [riot-tag="msg-usr"] .msg_bubble_container{ overflow: hidden; } msg-usr .msg_bubble, [riot-tag="msg-usr"] .msg_bubble{ display: inline-block; height:40px; line-height: 40px; padding: 0px 10px; border-radius: 10px; margin-top: 5px; color:white } msg-usr .msg_bubble_l, [riot-tag="msg-usr"] .msg_bubble_l{ background:#009926; float:left } msg-usr .msg_bubble_r, [riot-tag="msg-usr"] .msg_bubble_r{ background:#C3C3C3; float:right }',function(){function e(e){console.log(e),t.msgs.unshift(e),t.content.value="",t.update()}function n(e){t.msgs=e,t.trigger("view-route-to")}var s,t=nest.presentable(this),o=domain.action("MsgInClazzAddAction"),i=domain.action("MsgToUserLoadAction");t.msgs=[],t.userId,this.sendToStu=function(e){o.execute({channel:s,content:t.content.value}),e.stopPropagation()}.bind(this),this.on("open",function(e){t.userId=e.userId,s=util.getRelationShip(t.userId,__page.user._id),i.execute({channel:s,to:t.userId})}),this.on("mount",function(){o.onDone(e),i.onDone(n)})});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var domain = __webpack_require__(3);
...
/***/ function(module, exports, __webpack_require__) {
	...
	domain.action('StudentSignUpAction').onExecute(function(data){
	    apiFactory.put('/student').drive(this).send(data);
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ajax = $ && $.ajax || __webpack_require__(2);
